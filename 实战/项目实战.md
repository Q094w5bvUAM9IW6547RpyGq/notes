...
>
>  [思否参考文章](https://segmentfault.com/a/1190000038371791)
>
>  CSDN[参考文章](https://blog.csdn.net/j_bleach/article/details/106891503?utm_medium=distribute.pc_relevant.none-task-blog-title-1&spm=1001.2101.3001.4242)
...
1. **滚动过快，出现白边**
   ...
   > 1.上下预留一定的高度，但是在火狐浏览器测试中，滚动过快还是会有白边，如果继续增加预留高度的话，渲染的数据开始多了，不符合项目需求
   >
   > 2.使用transform,但是出现样式错乱问题，且依然会出现白边
   >
   > ![image-20220313171543207](D:\工作\notes\实战\项目实战.assets\image-20220313171543207.png)

   ```js
    const mainTable = this.$refs.multipleTable.$el.getElementsByClassName('el-table__body')
    // el-table__body包含左侧非固定部分，右侧固定部分
    Array.from(mainTable).forEach(table => {
           table.style.height = height + 'px' // 可滚动区域
           console.log('table',table.children[1])
           const tbody = table.children[1]
           tbody.style.position = 'absolute'
           if (clientHeight < height && clientHeight + scrollTop + (this.reserveTop  * itemSize) + itemSize >= height) {
             // 由于el-table 在滚动到最后时，会出现抖动，因此增加判断，单独设置属性
             // table.style.paddingTop = (scrollTop - itemSize) <= 0 ? 0 + 'px' : (scrollTop - itemSize) + 'px'
             // table.style.paddingBottom = 0
             tbody.style.transform = `translate3d(0,${this.startOffset + itemSize}px,0)`;
             console.log('滚动到底了')
           } else {
             console.log('还未滚动到底')
             tbody.style.transform = `translate3d(0,${this.startOffset}px,0)`;
             // table.style.paddingTop = scrollTop + 'px' // 已浏览区域
             // table.style.paddingBottom = height - scrollTop - ((bufferCount + this.reserveBottom) * itemSize) + 'px'// 待浏览区域
           }
         })
   ```

   > 空白问题/拖动不流畅均是因为`渲染性能低下`导致的

2. ref不唯一，导致获取表格失败

   > 一定要确保ref是唯一的，不然会出现一些无法预估的问题

3. 滚动条滚动到底的时候，出现抖动

   > 1.每次滚动方法执行最后，获取开始索引，和最后索引，再次执行方法，比较旧的起始索引和新的起始索引 并 旧的结束索引和新的结束索引是否一致，一致就return
   >
   > 2.一定要固定每行的高度，el-table似乎会有一个最小的行高，我通过 :row-style={height: '35'}，设置行高，但是在火狐浏览器中，滚动到最底部时，行高变高了

4. tab切换，如果数据过少，数据不会显示，需要滚动才显示

   > 监听数据变化，tab切换数据变化，重置scrollTop为0

   ## Vue+ElementUI实现el-table内容的导出

   > [参考文章](https://juejin.cn/post/7036733493161754660)
